...
export function DocumentStudioPage() {
  ...
  const addModule = (kind: ModuleKind) => {
    ...
    addHistory(`Inserted ${next.title}`);
  };
  ...
  const applyTemplate = (templateId = selectedTemplateId) => {
    ...
    addHistory(`Applied ${template.name}`);
  };
  ...
  const finishWizard = () => {
    ...
    if (startMode === 'blank') {
      ...
      if (saved) {
        try {
          const parsed = JSON.parse(saved) as { modules?: StudioModule[] };
          setModules(cloneModules(parsed.modules ?? []));
        } catch {
          setModules([]);
        }
      } else {
      ...
      if (wizardAnswers.customerLogo && !nextModules.some((module) => module.kind === 'image')) addModuleToList(nextModules, 'image');
    ...
    setWizardStep(1);
    setSelectedId(null);
    addHistory('Built document from Setup Wizard');
  };
  ...
  const saveLayout = () => {
    ...
    addHistory('Saved custom template');
  ...
  const startDrag = (event: PointerEvent<HTMLDivElement>, module: StudioModule) => {
    ...
    const move = (moveEvent: globalThis.PointerEvent) => {
      ...
      if (snapX !== undefined) nextX = snapX;
      if (snapY !== undefined) nextY = snapY;
      setGuides({ x: snapX !== undefined ? snapX : undefined, y: snapY !== undefined ? snapY : undefined });
      setModules((current) => current.map((item) => item.id === module.id ? { ...item, x: Math.round(nextX * 2) / 2, y: Math.round(nextY * 2) / 2 } : item));
    };

    const stop = () => {
      setGuides({});
      addHistory(`Moved ${module.title}`);
  ...
  const startResize = (event: PointerEvent<HTMLSpanElement>, module: StudioModule, direction: ResizeDirection) => {
    ...
    const stop = () => {
      addHistory(`Resized ${module.title}`);
      window.removeEventListener('pointermove', move);
      window.removeEventListener('pointerup', stop);
    };
    window.addEventListener('pointermove', move);
  ...
  return (
    <section className="document-studio-page">
      ...
      <div className="studio-command-bar">
        ...
        <div className="document-kind-switch"><button className={previewMode === 'template' ? 'is-active' : ''} onClick={() => setPreviewMode('template')} type="button">Template Mode</button><button className={previewMode === 'live' ? 'is-active' : ''} onClick={() => setPreviewMode('live')} type="button">Live Preview</button></div>
        <label className="studio-theme-control">Style<select value={theme} onChange={(event) => setTheme(event.target.value as ThemeName)}>{(['Corporate Blue', 'Industrial Gray', 'Minimal', 'Dark', 'Rounded', 'Sharp'] as ThemeName[]).map((name) => <option key={name}>{name}</option>)}</select></label>
      ...
      <div className={`studio-workspace has-library${selectedModule ? ' has-inspector' : ''}`}>
        <aside className="studio-module-library">
          <header><p className="eyebrow">Module library</p><h3>Add what you need</h3></header>
        ...
        <div className="studio-canvas-stage">
          <div className="studio-canvas-scale" style={{ width: `${zoom}%` }}>
            <div className={`studio-canvas theme-${theme.toLowerCase().replaceAll(' ', '-')}`} ref={canvasRef}>
              ...
              {modules.map((module) => (
                <div className={`studio-module studio-module-${module.kind}${selectedId === module.id ? ' is-selected' : ''}`} key={module.id} onPointerDown={(event) => startDrag(event, module)} style={{ left: `${module.x}%`, top: `${module.y}%`, width: `${module.width}%`, height: `${module.height}%` }}>
                  <div className="studio-module-handle"><span>•••</span><small>{module.title}</small></div>
                  <div className="studio-module-preview">{modulePreview(module)}</div>
                  {selectedId === module.id && resizeDirections.map((direction) => <span className={`studio-resize-handle is-${direction}`} key={direction} onPointerDown={(event) => startResize(event, module, direction)} />)}
                </div>
              ))}
            </div>
          </div>
      ...
      {templateOpen && (
        <div className="studio-modal-backdrop">
          <section className="studio-template-modal">
            <header><div><p className="eyebrow">Choose a starting point</p><h2>Template Gallery</h2><p>Select a layout that already feels close, then make it yours.</p></div><button onClick={() => setTemplateOpen(false)} type="button">×</button></header>
            <div className="template-gallery">{visibleTemplates.map((template) => <button className={selectedTemplateId === template.id ? 'is-selected' : ''} key={template.id} onClick={() => setSelectedTemplateId(template.id)} type="button"><div className="template-thumbnail">{template.modules.map((module) => <span key={module.id} style={{ left: `${module.x}%`, top: `${module.y}%`, width: `${module.width}%`, height: `${module.height}%` }} />)}</div><strong>{template.name}</strong><small>{template.description}</small><em>Best for: {template.bestFor}</em><span>{template.modules.length} modules included</span></button>)}</div>
            <footer><span>Compare the layouts, then open the closest fit.</span><button className="studio-primary-button" onClick={() => applyTemplate()} type="button">Use This Template</button></footer>
          </section>
        </div>
      )}
      ...
      {wizardOpen && (
        <div className="studio-modal-backdrop">
          <section className="studio-wizard">
            <header><div><p className="eyebrow">Welcome to GraphicsFlow</p><h2>Let’s build your first document.</h2><p>Answer a few questions and GraphicsFlow will create a professional starting point for you.</p></div><button onClick={() => setWizardOpen(false)} type="button">×</button></header>
            <div className="wizard-progress">{['Document', 'Starting point', 'Industry', 'Information', 'Features', 'Template'].map((label, index) => <span className={wizardStep >= index + 1 ? 'is-active' : ''} key={label}><i>{index + 1}</i>{label}</span>)}</div>
            <div className="wizard-body">
              {wizardStep === 1 && <div className="wizard-step"><p className="wizard-kicker">What are you creating?</p><h3>Choose your document type.</h3><div className="wizard-choice-grid"><button className={wizardKind === 'approval' ? 'is-selected' : ''} onClick={() => { setWizardKind('approval'); setWizardTemplateId('modern-minimal'); }} type="button"><strong>New Approval</strong><span>Customer-facing artwork review and sign-off.</span></button><button className={wizardKind === 'printCard' ? 'is-selected' : ''} onClick={() => { setWizardKind('printCard'); setWizardTemplateId('corrugated'); }} type="button"><strong>New Print Card</strong><span>Production-focused specifications and job details.</span></button></div></div>}
              {wizardStep === 2 && <div className="wizard-step"><p className="wizard-kicker">How would you like to start?</p><h3>Choose the path that feels easiest.</h3><div className="wizard-choice-grid four">{([['recommended', 'Recommended Template', 'Answer a few questions and start 90% finished.'], ['blank', 'Blank Layout', 'Start fresh with an empty printable canvas.'], ['import', 'Import Existing Template', 'Bring in a previously exported GraphicsFlow template.'], ['duplicate', 'Duplicate My Template', 'Use your most recently saved custom layout.']] as Array<[StartMode, string, string]>).map(([mode, title, text]) => <button className={startMode === mode ? 'is-selected' : ''} key={mode} onClick={() => setStartMode(mode)} type="button"><strong>{title}</strong><span>{text}</span></button>)}</div></div>}
              {wizardStep === 3 && <div className="wizard-step"><p className="wizard-kicker">Tell us about your work.</p><h3>What industry best matches your workflow?</h3><div className="wizard-pill-grid">{industryOptions.map((industry) => <button className={wizardAnswers.industry === industry ? 'is-selected' : ''} key={industry} onClick={() => setWizardAnswers((current) => ({ ...current, industry }))} type="button">{industry}</button>)}</div></div>}
              {wizardStep === 4 && <div className="wizard-step"><p className="wizard-kicker">What information matters?</p><h3>Select what you normally include.</h3><div className="wizard-check-grid">{fieldOptions.map((field) => <label key={field}><input checked={wizardAnswers.fields.includes(field)} onChange={(event) => setWizardAnswers((current) => ({ ...current, fields: event.target.checked ? [...current.fields, field] : current.fields.filter((item) => item !== field) }))} type="checkbox" /><span>{field}</span></label>)}</div></div>}
              {wizardStep === 5 && <div className="wizard-step"><p className="wizard-kicker">Finish the setup.</p><h3>Which smart features should we include?</h3><div className="wizard-toggle-list"><label><span><strong>Signatures</strong><small>Add customer sign-off fields.</small></span><input checked={wizardAnswers.signatures} onChange={(event) => setWizardAnswers((current) => ({ ...current, signatures: event.target.checked }))} type="checkbox" /></label><label><span><strong>Revision History</strong><small>Track revision number, date, and changes.</small></span><input checked={wizardAnswers.revisionHistory} onChange={(event) => setWizardAnswers((current) => ({ ...current, revisionHistory: event.target.checked }))} type="checkbox" /></label><label><span><strong>Customer Logo</strong><small>Reserve a smart image module for branding.</small></span><input checked={wizardAnswers.customerLogo} onChange={(event) => setWizardAnswers((current) => ({ ...current, customerLogo: event.target.checked }))} type="checkbox" /></label><label><span><strong>Print Card Workflow</strong><small>Prepare this setup for linked Print Cards.</small></span><input checked={wizardAnswers.needsPrintCard} onChange={(event) => setWizardAnswers((current) => ({ ...current, needsPrintCard: event.target.checked }))} type="checkbox" /></label></div></div>}
              {wizardStep === 6 && <div className="wizard-step"><p className="wizard-kicker">Your recommended starting points</p><h3>Pick the one that feels most like you.</h3><div className="wizard-template-grid">{wizardTemplates.map((template) => <button className={wizardTemplateId === template.id ? 'is-selected' : ''} key={template.id} onClick={() => setWizardTemplateId(template.id)} type="button"><div className="template-thumbnail">{template.modules.map((module) => <span key={module.id} style={{ left: `${module.x}%`, top: `${module.y}%`, width: `${module.width}%`, height: `${module.height}%` }} />)}</div><strong>{template.name}</strong><small>{template.description}</small><em>{template.bestFor}</em></button>)}</div></div>}
            </div>
            <footer><button className="studio-secondary-button" disabled={wizardStep === 1} onClick={() => setWizardStep((step) => Math.max(1, step - 1))} type="button">Back</button><span>Step {wizardStep} of 6</span>{wizardStep < 6 ? <button className="studio-primary-button" onClick={() => { if (wizardStep === 2 && startMode !== 'recommended') setWizardStep(6); else setWizardStep((step) => Math.min(6, step + 1)); }} type="button">Continue</button> : <button className="studio-primary-button" onClick={finishWizard} type="button">Build My Document</button>}</footer>
          </section>
        </div>
      )}
    </section>
