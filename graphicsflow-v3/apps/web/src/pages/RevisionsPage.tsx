    ...
    <section className="revisions-page">
      ...
      {record && !query.isFetching && <div className="revision-workspace"><div className="revision-main-grid">
        <div className="revision-history-column">
          <section className="revision-record-hero"><div><span className="revision-document-label">{record.documentType === 'approval' ? 'Approval' : 'Print Card'}</span><h3>{record.documentType === 'approval' ? formatGNumber(record.gNumber) : formatSpecNumber(record.specificationNumber)}</h3><p>{record.documentType === 'printCard' && `${formatGNumber(record.gNumber)} · `}{record.customerName} · {record.partNumber}</p></div><div className="revision-current-summary"><span>Current revision</span><strong>{record.currentRevision?.revisionLabel || '—'}</strong><small>{record.currentRevision ? displayDate(record.currentRevision.createdAt || record.currentRevision.revisionDate) : 'Not recorded'}</small></div></section>
