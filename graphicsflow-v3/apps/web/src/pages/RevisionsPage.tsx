...
export function RevisionsPage() {
  ...
  const query = useQuery({ queryKey: ['revision-lookup', type, search], queryFn: () => lookup(type, search), enabled: Boolean(search), retry: false, refetchOnWindowFocus: false });
  ...
  const viewerRecord: GraphicRecord | null = record ? { id: record.graphicId, gNumber: record.gNumber, customerNumber: record.customerNumber, customerName: record.customerName, specificationNumber: record.specificationNumber, partNumber: record.partNumber, previewImage: null, createdAt: null, source: 'graphicsflow', canDelete: false } : null;
      ...
      {record && <div className="revision-workspace"><div className="revision-main-grid">
        <div className="revision-history-column">
          ...
          <section className="revision-journey"><header><div><p className="eyebrow">Revision journey</p><h3>The life of this {record.documentType === 'approval' ? 'Approval' : 'Print Card'}</h3></div><span>{record.journey.length} revision{record.journey.length === 1 ? '' : 's'}</span></header>
            {record.journey.length === 0 ? <div className="revision-journey-empty">No structured revisions have been recorded.</div> : <ol>{record.journey.map((revision, index) => { const isSelected = selectedRevisionIndex === index; return <li className={`${revision.isCurrent ? 'is-current ' : ''}${isSelected ? 'is-selected' : ''}`.trim()} key={`${revision.id ?? 'legacy'}-${index}`}><div className="revision-node"><span>{revision.revisionLabel || index}</span></div><article aria-pressed={isSelected} onClick={() => selectRevision(index)} onKeyDown={(event) => handleRevisionCardKeyDown(event, index)} role="button" tabIndex={0}><header><div><strong>Revision {revision.revisionLabel || index}</strong>{revision.isCurrent && <em>Current</em>}</div><time>{displayDate(revision.revisionDate || revision.createdAt)}</time></header><p>{revision.description || 'No change description was recorded.'}</p><footer><span>{revision.source === 'legacy-import' ? 'Legacy history' : 'GraphicsFlow'}</span><span>{[revision.csr, revision.designer].filter(Boolean).join(' · ') || 'Author not recorded'}</span><span className="revision-card-action">{isSelected ? 'Selected' : 'Click to view'}</span></footer></article></li>; })}</ol>}
          </section>
        </div>
        {record.documentType === 'printCard'
          ? <PrintCardRevisionWorkspace record={record} selectedRevision={selectedRevision} selectedRevisionIndex={selectedRevisionIndex} viewerError={viewerError} viewerLoading={viewerLoading} onOpenCurrent={() => void openCurrent()} onRevisionSaved={() => void query.refetch()} />
