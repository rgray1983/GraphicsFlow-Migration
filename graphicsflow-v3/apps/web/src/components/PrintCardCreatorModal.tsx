        ...
        <form className="print-card-creator" onSubmit={submit}>
          <section className="creator-editor">
            ...
            <section className="creator-work-section creator-artwork-section">
              ...
              {artworkMatches && <section className="creator-artwork-matches"><div><span className="creator-kicker">Available Artwork PDFs</span><h4>{artworkMatches.message}</h4></div>{artworkMatches.matches.length > 0 && <div className="creator-artwork-list" role="listbox" aria-label="Available artwork PDFs">{artworkMatches.matches.map((match) => {
                const selected = draft.liveArtworkRelativePath === match.relativePath && !draft.artPdfBase64;
                return <button aria-selected={selected} className={`creator-artwork-row${selected ? ' is-selected' : ''}`} key={match.relativePath} onClick={() => void chooseLiveArtwork(match)} role="option" type="button"><span className="creator-artwork-found" aria-hidden="true">✓</span><strong>{match.name}</strong><span>{formatMatch(match)}</span><small title={match.relativePath}>{match.relativePath}</small></button>;
              })}</div>}</section>}
