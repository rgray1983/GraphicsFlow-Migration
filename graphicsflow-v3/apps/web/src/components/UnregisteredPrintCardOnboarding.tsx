...
export function UnregisteredPrintCardOnboarding({ printCard, onCreated }: Props) {
  ...
  return <div className="revision-onboarding-grid">
    <section className="revision-onboarding-info">
      ...
      <p>Enter all known historical revisions so the next Print Card continues from the correct revision.</p>
    </section>
    <section className="revision-onboarding-workspace">
      ...
      <form className="revision-onboarding-form" onSubmit={submit}>
        <header><div><p className="eyebrow">Create or link record</p><h3>Connect this live Print Card<Tooltip content={<><strong>Existing G#:</strong> Enter an existing G# to link this Print Card. The saved customer and part information for that G# will be used, and the disabled fields below will not overwrite it.<br /><br /><strong>New G#:</strong> Leave Existing G# blank and GraphicsFlow will create the next G# automatically using the customer and part information below.</>} label="How Create or Link Print Card works" position="right" /></h3></div><button disabled={saving} type="submit">{saving ? 'Connecting…' : 'Create/Link Print Card'}</button></header>
        <div className="revision-onboarding-fields">
          <label><span className="field-label-line">Existing G# <small className="field-label-optional">optional</small></span><input placeholder="Leave blank to create a new G#" value={form.gNumber} onChange={(event) => setForm({ ...form, gNumber: event.target.value })} /></label>
