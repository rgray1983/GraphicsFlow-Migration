...
export function ApprovalRevisionRegenerateModal({ graphicId, revisionId, revisionLabel, isOpen, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    let objectUrl = '';
    setLoading(true); setClosing(false); setError(null); setResult(null);
  ...
  const finish = async () => {
    if (!revisionId || closing) return;
    setClosing(true);
    try {
      await fetch(`/api/graphics/${graphicId}/approval/revisions/${revisionId}.pdf?download=1`, { cache: 'no-store' });
    } finally {
      onClose();
      setClosing(false);
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={loading || closing ? () => undefined : () => void finish()} title={`Regenerate Approval · Revision ${revisionLabel}`}>
      <div className="approval-regenerate-result">
        ...
        {!loading && result && <>
          ...
          <footer><button disabled={closing} onClick={() => void finish()} type="button">{closing ? 'Cleaning Up…' : 'Done'}</button></footer>
        </>}
